...
export async function exportCharacterPDF(character) {
  ...
  const hasDevinne = await loadFont(doc, '/assets/fonts/DevinneSwash.ttf', 'DevinneSwash.ttf', 'DevinneSwash');
  const hasInk     = await loadFont(doc, '/assets/fonts/Inkfree.ttf', 'Inkfree.ttf', 'Inkfree');
  const serifFont  = hasDevinne ? 'DevinneSwash' : 'helvetica';
  const inkFont    = hasInk     ? 'Inkfree'      : 'helvetica';
